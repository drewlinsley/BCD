...
4. **Resolver** — retrieves candidates by product *and* producer name (trigram on Postgres, fuzzy tokens on SQLite), scores each against its full identity with generic label words down-weighted, and returns a **verdict per object**: `resolved` (floor + margin — safe to overlay), `ambiguous` (a shortlist), or `unresolved`. Each resolved candidate gets a **personal score** from the user's `TasteProfile`; cold products are scored from **chemistry alone** (the moat) and flagged.
...
| Resolve + score | [services/api/resolver.py](../services/api/bcd_api/resolver.py), [matching.py](../services/api/bcd_api/matching.py) |
